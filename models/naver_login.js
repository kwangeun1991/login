...
const naverLogin = {
  clientId : '9FAfed36vKxds7VvKwOq',
  secret : 'L34yQf822h',
  redirectUri : 'http://kwang4585.xyz:3000/member/login_callback',
  ...
  checkExists : async function(code, state, req) {
    ...
    if (accessToken) {
      ...
      if (profile) {
        ...
        const sql = `SELECT * FROM member WHERE snsType='naver' AND snsId = :snsId`;
  ...
  */
  login : async function (req, res) {
    /**
    * snsType - naver
    * req.session.naberProfile.id 00 snsId
    *
    */
    ...
    try {
      ...
      if (rows.length == 0) {
        throw new Error("네이버로그인 - 존재하지않는 snsId");
      }
